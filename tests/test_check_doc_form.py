...
def _install_checker(repo: Path) -> None:
    ...
    # check_doc_form.py가 이제 gate_config.py를 import한다 — 없으면 격리
    # 저장소에서 ModuleNotFoundError로 죽는다(실제 배포와 같은 배선).
    shutil.copy2(_REPO_ROOT / "scripts" / "gate_config.py", repo / "scripts" / "gate_config.py")
    _copy_forms(repo)
