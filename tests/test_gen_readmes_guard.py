# BLUF: gen_readmes의 자동생성 블록 파괴 방지와 gitignore 폴더 제외를 무DB·무LLM으로 검증하는 회귀.
"""gen_readmes의 자동생성 블록 파괴 방지 회귀.

블록(BLUF-INDEX)은 재생성 때 통째로 교체된다 — 그 안에 손으로 쓴 줄은 말없이
사라지고, 사라지면 drift도 같이 없어져 커밋 게이트가 울릴 근거조차 남지 않는다.
그래서 파괴 전에 멈춰야 하고, 이 파일이 그 성질을 고정한다.
