...
# **줄 시작 앵커가 핵심이다.** 종전엔 `MARK_START in content` 단순 포함으로
# 봤는데, 이 마커 문법 자체를 설명하는 산문 문장이 본문 중간에 그대로 인용되면
# 그 지점이 블록 시작으로 오인됐다(실사고: docs/history/
# B-autogen-marker-substring-match-hides-violations.md). 같은 처방을
# check_doc_form.py의 `_AUTOGEN_START`/`_AUTOGEN_END`가 이미 받았다 — 그 파일을
# import하지는 않는다(두 게이트 파일은 서로 안 걸치는 격리 설계), 정규식 방식만
# 미러한다. 문장 중간 인용은 줄을 시작하지 않으므로 이 앵커에 안 걸린다.
