#!/usr/bin/env python3
# BLUF: 산문 형태(shape) 판정 — 펜스·문장 경계·체크박스·이슈 참조·예산 파싱 문법(stdlib only, 의존성 0) — 게이트들이 순환 임포트 없이 공유하는 leaf 모듈.
"""줄 형태(shape) 판정 — 펜스·문장 경계·체크박스·이슈 참조·예산 파싱 문법.

게이트들과 저장소별 설정이 함께 쓰는 leaf 모듈이다(소비처는 이 이름들을
import하는 쪽을 찾으면 나온다 — 여기 열거하지 않는다). 애초엔 체크박스·이슈
참조 판정만 있었고, 그 하나의 소비처를 위해 두 게이트 모듈이 서로를 참조하는
순환을 피하는 것이 동기였다. 이후 게이트 파일에 전재돼 있던 펜스·문장 경계
판정과 예산 파싱 정규식도 이관해, "산문 형태 판정"이라는 이 모듈의 선언을
실제로 채웠다.
