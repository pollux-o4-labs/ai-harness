...
vgo 적용: BLUF 마커를 모아 폴더 README 인덱스를 결정적으로 생성 — Neo4j/Qdrant/claude-p 불요.
...
# 대상에서 제외한다(vgo가 관리하는 마커 블록만 실림, BLUF 요구 없음).
# 주: onboard.py에도 동일 상수(AGENT_CONFIG_NAMES)를 둔다 — gen_readmes는 의존성
# 경량(LLM 0, standalone) 유지 위해 vgo 패키지를 import하지 않으므로 의도적 미러.
AGENT_CONFIG_NAMES = {"AGENTS.md", "CLAUDE.md"}

MARK_START = "<!-- BLUF-INDEX:START — auto-generated by scripts/gen_readmes.py; 이 블록은 수기 편집 금지 -->"
...
def is_git_ignored(path: Path) -> bool:
    ...
    cwd는 대상 경로의 부모(그 파일이 속한 git repo) — REPO_ROOT 고정이면 `--root`가
    vgo repo 밖(예: 미러 클론)일 때 vgo의 .gitignore로 오판한다(ADR 0021 F3)."""
