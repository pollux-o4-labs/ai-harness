...
**모드**:
  ...
  python scripts/check_pr_body.py --merge-check <PR>     # 머지 준비 dry-run(실제
                                                         # 머지 안 함, exit 1 = 미준비)
...
자체를 부르지 않는다 — 체크리스트 전량 채움 + 규칙 11(docs/rules/11-review-evidence-record.md —
repo별 특화 지점, 이 저장소엔 아직 없다. README가 안내)이 요구하는 리뷰 종합
...
구조는 그대로 강제하고(`docs/docs-format/pr-comment.md`의 줄수·줄자수 예산),
내부 용어 풀이(`JARGON_TERMS`)도 그대로 적용한다 — 체크리스트의 은어 검수
항목은 "PR에 작성된 글" 전체가 대상이고 코멘트도 그 글이기 때문이다.

  gh pr create    섹션·예산·용어·체크절(존재만)
  gh pr merge     위 + 체크 전량
  gh pr comment   줄자수·줄수 예산 + 한 줄 한 문장 + 용어 풀이(섹션·체크리스트 없음)

(comment 예산 수치는 `docs/docs-format/pr-comment.md`가 정본 — 여기 재서술
 안 함. 이 폼 파일은 저장소마다 따로 작성한다 — 아래 `_COMMENT_FORM_PATH` 주석.)
"""
...
# `gh pr comment` 예산의 정본 — 이 dict는 이 파일에 없다(규칙 08 제3조).
# scripts/check_doc_form.py의 _BUDGET_PATS와 같은 문구 관례를 파싱하지만,
# 두 스크립트는 stdlib only 제약(훅에서 vgo 설치 없이 돈다)이라 import로 못
# 합쳐 정규식을 복제한다(위 _SENTENCE_END와 같은 이유).
#
# `docs/docs-format/pr-comment.md`·아래 check_review_evidence가 참조하는
# `docs/rules/11-review-evidence-record.md`는 이 기능을 처음 만든 저장소의
# 경로를 그대로 옮겨온 것이다 — **repo별 특화 지점**(README가 안내)이라 이
# 저장소엔 아직 없을 수 있다. 없으면 폼을 못 뽑아 fail-closed로 리젝하는 것이
# 설계대로다(우회가 아니라 "아직 안 채운 설정"임을 알리는 것) — 저장소마다
# 이 경로에 폼 파일을 작성하면 그때부터 코멘트 게이트·리뷰 근거 검사가 켜진다.
_COMMENT_FORM_PATH = (
...
# (`## 리뷰 종합 — 2차 (8c8f4f7)`)에서 뽑는다. 여기 하드코딩하지 않는 이유는
# _COMMENT_BUDGET_PATS와 같다(규칙 08 제3조) — 헤더 관례가 바뀌면 폼 파일만
...
def load_comment_budgets() -> dict[str, int]:
    """`docs/docs-format/pr-comment.md`에서 코멘트 예산을 뽑는다.

    폼이 정본이라 수치를 여기 하드코딩하지 않는다(규칙 08 제3조). 못 뽑으면
    빈 dict를 반환한다 — fail-closed 판단은 check_comment가 한다.
    """
...
def load_review_header_prefix() -> str | None:
    """`docs/docs-format/pr-comment.md`의 예시 문구에서 리뷰 종합 헤더 접두어를
    뽑는다(`"## 리뷰 종합 — 2차 (8c8f4f7)"` → `"## 리뷰 종합"`).

    load_comment_budgets와 같은 이유로 토큰을 이 파일에 재서술하지 않는다(규칙
    08 제3조). 못 뽑으면 None — fail-closed 판단은 호출자(check_review_evidence)가
    한다.
    """
    if not _COMMENT_FORM_PATH.is_file():
        return None
...
def _report_comment(violations: list[str], body: str) -> None:
    ...
    print(
        "\n형식: docs/docs-format/pr-comment.md",
        file=sys.stderr,
    )
...
def check_review_evidence(comments: list[dict], head_sha: str) -> list[str]:
    """리뷰 종합 코멘트의 존재·신선도를 검사한다(규칙 11 제1조 "근거 없는 체크
    금지"를 처음 기계로 태우는 백스톱 — docs/rules/11-review-evidence-record.md,
    repo별 특화 지점. README가 안내하며, 이 저장소엔 아직 없을 수 있다).
    ...
    `docs/docs-format/pr-comment.md`에서 뽑는다(규칙 08 제3조). 못 뽑으면
    잴 자가 없어서 fail-closed로 리젝한다(check_comment의 예산 미검출과 같은
    처방).
    """
    ...
    if not head_sha or not head_sha.lower().startswith(comment_sha.lower()):
        return [
            ...
            f"코멘트를 달아라(규칙 11 제5조 — 판정마다 새 코멘트)."
        ]
    return []
