...
(comment 예산 수치는 `docs/docs-format/pr-comment.md`가 정본 — 여기 재서술 안 함.)

**레포별 설정은 `gate_config.py`에 있다** — 은어 목록·면제 섹션·규칙 인용처럼
저장소마다 달라야 하는 값은 이 파일에 안 둔다(그래야 이 파일 자체는 어느
저장소에 복사해도 바이트가 같다, `scripts/check_gate_drift.py`가 그 전제를
검사한다).
"""
...
# `gh pr comment` 예산의 정본 — 이 dict는 이 파일에 없다. scripts/check_doc_form.py의
...
def check_review_evidence(comments: list[dict], head_sha: str) -> list[str]:
    ...
    `docs/docs-format/pr-comment.md`에서 뽑는다. 못 뽑으면 잴 자가 없어서
