...
레포 공용 **문서·PR 게이트 일습**(pre-commit + PR 훅). `claude -p`나 별도
지식베이스 없이 도는 **stdlib only** 파이썬이라, 어느 레포에 clone/복사해도
바로 붙는다. 게이트 설계·근거는 실제 프로젝트 운영 경험에서 뽑아왔고, 여기
실린 설정값은 그 실사용 예다.
...
- `JARGON_TERMS` — 풀이를 강제할 내부 은어.
  **여기 실린 목록은 원 사용 레포의 예시**라 대상 레포 은어로 갈아야 한다(상습범만 등재).
...
- repo에서 급히 고친 개선은 되돌려 이 정본에도 반영해야 한다.
  안 하면 정본이 뒤처지고 사본마다 갈라진다(정본과 사본이 양방향으로 갈라진 전례가 있다).
